...
- `category`: `'matchResults' | 'evaluations' | 'calendarResponses'`
...
- Per ogni annata selezionata, fetch dei dati di quella categoria specifica (endpoint esistente di lettura, cambiando header `X-Annata-Id` alla chiamata), SENZA modificare `window.matchResults` / `window.evaluations` / `window.calendarResponses` globali (che restano quelli della stagione corrente).
- I dati fusi vivono in una variabile locale al componente (es. `comparisonData`), usata solo per il render della lista/tabella di quella sezione.
...
- Alla conferma: POST all'endpoint di cancellazione; in caso di successo, rimuove l'annata dal dropdown/selezione e dalla vista; mostra un messaggio di conferma con il conteggio di elementi cancellati.
...
Headers: X-Auth-Session (sessione admin)
Body: { targetAnnataId: string, category: 'matchResults'|'evaluations'|'calendarResponses' }
```

Logica:
1. Richiede sessione valida e `role === 'admin'` → 403 altrimenti.
2. Valida `category` contro whitelist `['matchResults', 'evaluations', 'calendarResponses']` → 400 se non valida (nessun'altra categoria è raggiungibile da questo endpoint, a differenza di `season-restore` che ne copre 7 — qui il set è deliberatamente ristretto alle 3 sezioni di questa spec).
...
5. Verifica che `targetAnnataId` non sia l'annata correntemente attiva per la sessione (`annataId` dell'header) → 400 "Non puoi cancellare l'annata attiva da qui, usa Cambio Stagione" (protezione contro cancellazioni accidentali della stagione in corso).
6. Legge la chiave corrente (`annate:<targetAnnataId>:<category>`), conta gli elementi (per `matchResults`/`trainingSessions`: `Object.keys(...).length`; per `evaluations`/`calendarResponses`: somma degli atleti in tutte le date) per il messaggio di riepilogo.
7. Sovrascrive la chiave con struttura vuota (`{}` — tutte e 3 le categorie sono oggetti keyed, non array, quindi `{}` è coerente in tutti i casi).
...
- Script Node isolato che simula `wipe-annata-category` con mock KV: verifica whitelist categoria, isolamento società, blocco su annata attiva, conteggio corretto, chiave svuotata correttamente per ciascuna delle 3 categorie.
