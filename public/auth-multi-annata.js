...
async function showUserModal(userData = null) {
    ...
    document.getElementById('user-role').addEventListener('change', (e) => {
        ...
        annateContainer.style.display = e.target.value === 'coach' ? 'block' : 'none';
    });
    ...
    document.getElementById('user-form').addEventListener('submit', async (e) => {
        ...
        const annateSelezionate = role === 'coach' 
            ? Array.from(document.querySelectorAll('input[name="annate"]:checked')).map(cb => cb.value)
            : [];
        
        if (role === 'coach' && annateSelezionate.length === 0) {
            alert('⚠️ Seleziona almeno un\'annata per il Coach');
            return;
        }
        ...
        try {
            const response = await fetch('/api/auth/' + (isEdit ? 'update-user' : 'create-user'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            document.body.removeChild(modal);
            alert(`✅ Utente ${isEdit ? 'modificato' : 'creato'} con successo!`);
            loadUsersList();
...
window.deleteUser = async function(username) {
    if (!confirm(`⚠️ Vuoi eliminare l'utente "${username}"?\n\nQuesta operazione è IRREVERSIBILE!`)) {
        return;
    }
