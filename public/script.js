...
dati è richiesta una password.</p><form id="password-form"><div class="mb-3"><label for="password-input" class="form-label">Password</label><input type="password" class="form-control" id="password-input" required><div id="password-error" class="text-danger mt-2" style="display: none;">Password non corretta.</div></div><button type="submit" class="btn btn-primary-custom w-100">Accedi</button></form></div></div></div></div>`;
 ...
 prevMonthBtn: document.getElementById('prev-month-btn'),
        ...
        availableList: 
 document.getElementById('available-list'),
        ...
        topAssistersContainer: document.getElementById('top-assisters-container'), // ✅ AGGIUNTA
 ...
 updateAllUI();
    ...
    const gpsFieldsForDisplay = { 'tipo_sessione':'Tipo', 'data_di_registrazione': 'Data', 'ora_registrazione': 'Ora', 'distanza_totale': 'Dist.
Totale (m)', 'tempo_totale': 'Tempo (min)', 'distanza_per_minuto':'Dist/min (m)', 'distanza_sprint': 'Distanza Sprint (m)', 'velocita_massima': 'Vel. Max (km/h)', 'numero_di_sprint': 'Num.
Sprint', 'max_acc': 'Max Acc (g)o(n°)', 'max_dec': 'Max Dec (g)o(n°)', 'passaggi_piede_sinistro':'Passaggi SX', 'passaggi_piede_destro':'Passaggi DX', 'cross_piede_sinistro':'Cross SX', 'cross_piede_destro':'Cross DX', 'potenza_massima_di_tiro':'Pot.
Tiro (km/h)', 'tiri_piede_sx': 'Tiri Piede SX', 'tiri_piede_dx': 'Tiri Piede DX', 'perc_passaggi_brevi': '% Passaggi Brevi', 'perc_lanci': '% Lanci', 'distanza_circuito': 'Dist.
Circuito (m)', 'tempo_circuito_totale_s': 'Tempo Circuito (s)', 'velocita_circuito': 'Vel. Circuito (km/h)', 'minuti_giocati': 'Minuti Giocati', 'gol': 'Gol', 'assist': 'Assist', 'ammonizioni': 'Gialli', 'espulsioni': 'Rossi', 'palle_recuperate': 'Palle Recuperate', 'palle_perse': 'Palle Perse', 'note': 'Note' };
    const radarMetrics = { 'distanza_sprint': 'Distanza Sprint', 'velocita_massima': 'Vel. Max', 'max_acc': 'Max Acc', 'max_dec': 'Max Dec', 'passaggi_piede_sinistro': 'Pass.
SX', 'passaggi_piede_destro': 'Pass. DX', 'cross_piede_sinistro': 'Cross SX', 'cross_piede_destro': 'Cross DX', 'potenza_massima_di_tiro': 'Pot.
Tiro', 'distanza_per_minuto': 'Dist/min', 'tiri_piede_sx': 'Tiri SX', 'tiri_piede_dx': 'Tiri DX', 'perc_passaggi_brevi': '% Pass. Brevi', 'perc_lanci': '% Lanci', 'velocita_circuito': 'Vel. Circuito' };
 ...
 v++;
        ...
        elements.statDr.className = dr > 0 ?
 'diff-pos' : (dr < 0 ? 'diff-neg' : '');
    };
    const updateAllUI = () => {
    ...
    const createTokenElement = (type, id) => {
        ...
        switch(type) {
            ...
            case 'opponent': token.innerHTML = '●'; token.classList.add('token-opponent');
                break;
        }
        return token;
    };
    const renderFormation = () => {
        ...
        const addExpiredIconIfNeeded = (athlete, jerseyElement) => {
            ...
            if (isExpired) {
                ...
                lockIcon.style.cssText = 'position: absolute; top: 0; left: 0; color: white; background-color: var(--primary-red); padding: 0.2em 0.4em;
border-radius: 50%; font-size: 1.5em;';
        ...
        athletes.forEach(athlete => {
            if (!placedAthleteIds.has(String(athlete.id))) {
                ...
                if (isExpired) { availablePlayer.classList.add('disabled'); availablePlayer.title 
= 'Visita medica scaduta. Atleta non schierabile.'; availablePlayer.innerHTML = `<span><strong>${athlete.number}.</strong> ${athlete.name}</span><span class="badge bg-danger"><i class="bi bi-lock-fill"></i></span>`; }
    ...
    const updateHomePage = () => {
        ...
        if (futureSessions.length > 0) {
            ...
            elements.homeNextSession.innerHTML = `<h5 class="card-title text-muted">PROSSIMA SESSIONE</h5><h6 class="mb-1">${nextSession.title}</h6><p class="mb-0">${sessionDate.toLocaleDateString('it-IT', { weekday: 'long', day: 'numeric', month: 'long' })}</p><p class="mb-0 text-muted">${nextSession.time ?
`Ore: ${nextSession.time}` : ''} ${nextSession.location ? `@ ${nextSession.location}` : ''}</p>`;
        } else {
        ...
        const monthlyScores = {}; athletes.forEach(a => monthlyScores[a.id] = { name: a.name, score: 0 });
        Object.entries(evaluations).forEach(([dateStr, dailyEvals]) => { if (dateStr.startsWith(currentMonthStr)) { Object.entries(dailyEvals).forEach(([athleteId, evaluation]) => { if (monthlyScores[athleteId]) { monthlyScores[athleteId].score += calculateAthleteScore(evaluation); } }); } });
        const sortedAthletes = Object.values(monthlyScores).sort((a, b) => b.score - a.score); if (sortedAthletes.length > 0 && sortedAthletes[0].score > 0) { const maxScore = sortedAthletes[0].score;
            const topPerformers = sortedAthletes.filter(athlete => athlete.score === maxScore); const names = topPerformers.map(p => p.name).join('<br>');
    ...
    // ✅ MODIFICA: renderAthletes aggiornata per mostrare l'icona del Vice Capitano
    const renderAthletes = () => {
        ...
        athletes.forEach(athlete => {
            ...
            const vcIcon = athlete.isViceCaptain ?
 '<i class="bi bi-star-half text-warning is-vice-captain"></i>' : '';
            card.innerHTML = `<div class="card athlete-card"><div class="card-body athlete-card-clickable" data-athlete-id="${athlete.id}"><img src="${athlete.avatar || defaultAvatar}" onerror="this.src='${defaultAvatar}'" alt="${athlete.name}" class="athlete-avatar me-3"><div><h5 class="card-title">${athlete.name} ${athlete.isCaptain ?
 '<i class="bi bi-star-fill is-captain"></i>' : ''} ${vcIcon}</h5><p class="card-text text-muted">${athlete.role}</p></div><div class="shirt-number">${athlete.number}</div>${statusIcon}</div><div class="card-actions no-print"><button class="btn btn-sm btn-outline-light gps-btn" title="Dati Performance" data-athlete-id="${athlete.id}"><i class="bi bi-person-fill-gear"></i></button><button class="btn btn-sm btn-outline-light edit-btn" title="Modifica Atleta" data-athlete-id="${athlete.id}"><i class="bi bi-pencil-fill"></i></button><button class="btn btn-sm btn-outline-light delete-btn" title="Elimina Atleta" data-athlete-id="${athlete.id}"><i class="bi bi-trash-fill"></i></button></div></div>`;
...
= session.title;
            ...
            if (matchResults) {
                Object.values(matchResults).forEach(match => {
                    ...
                    const myTeamName = "GO Sport";
                        ...
                        const location = match.location; // 'home' o 'away'
 ...
 (oppScore !== null && oppScore !== undefined && oppScore !== '');
                        if (isFuture && !hasScore) {
                            // Partita futura non giocata → verde scuro
                            matchEl.className = 'calendar-session session-partita-futura';
                        } else {
                            // Partita giocata (ou risultato inserito) → casa/trasferta
                            matchEl.className = location === 'home' ?
 'calendar-session session-partita-casa' : 'calendar-session session-partita-trasferta';
                        }
    ...
    const openSessionModal = (sessionData = null) => {
        ...
        if(sessionData) { document.getElementById('sessionModalLabel').textContent = "Modifica Sessione"; document.getElementById('session-id').value = sessionData.id; document.getElementById('session-date').value = sessionData.date; document.getElementById('session-title').value = sessionData.title; document.getElementById('session-time').value = sessionData.time;
            document.getElementById('session-location').value = sessionData.location; document.getElementById('session-goals').value = sessionData.goals; document.getElementById('session-description').value = sessionData.description; elements.deleteSessionBtn.style.display = 'block'; } else { document.getElementById('sessionModalLabel').textContent = "Pianifica Sessione";
            document.getElementById('session-id').value = ''; document.getElementById('session-date').valueAsDate = new Date(); elements.deleteSessionBtn.style.display = 'none'; } sessionModal.show();
    };
    // ✅ MODIFICA: Aggiornata renderMatchResults per colorare le card e mostrare gli assist
 ...
 `<div class="scorers-list"><i class="bi bi-futbol"></i> Marcatori: <strong style="color: var(--primary-red);">${myTeamScorers}</strong></div>` : ''}
    ...
    const renderCardsSummary = () => {
        ...
        allCards.forEach(card => {
            ...
            row.innerHTML = `<td>${athlete ? athlete.name : 'N/D'}</td><td>${card.type === 'yellow' ? '1' : '0'}</td><td>${card.type === 'red' ? '1' : '0'}</td><td>${new Date(card.date).toLocaleDateString('it-IT', {day: '2-digit', month: 'short'})}</td><td class="no-print"><button class="btn btn-sm btn-outline-danger py-0 px-1 remove-card-summary-row-btn" data-card-id="${card.uniqueId}"><i 
class="bi bi-x-lg"></i></button></td>`;
    ...
    // ✅ NUOVA FUNZIONE: Visualizza la classifica assistman
    const renderTopAssisters = () => {
        const assistCounts = {};
        Object.values(matchResults).forEach(match => {
        ...
        const sortedAssisters = Object.entries(assistCounts).map(([athleteId, assists]) => {
        ...
        if (sortedAssisters.length === 0) {
            elements.topAssistersContainer.innerHTML = '<p class="text-muted">Nessun assistman registrato.</p>';
            return;
        }

        let ol = '<ol class="list-group list-group-numbered">';
        sortedAssisters.forEach(assister => {
            ol += `<li class="list-group-item d-flex justify-content-between align-items-center" style="background: transparent; border-color: var(--border-color);">${assister.name}<span class="badge bg-danger rounded-pill">${assister.assists}</span></li>`;
        });
        ol += '</ol>';
        elements.topAssistersContainer.innerHTML = ol;
    };
    ...
    const updateMatchAnalysisChart = () => {
        ...
        if (opponentFilter !== 'all') {
            ...
            filteredMatches.forEach(match => {
                ...
                const myScore = match.location === 'home' ? match.homeScore : match.awayScore;
                const oppScore = match.location === 'home' ? match.awayScore 
 : match.homeScore;
        ...
        chartInstances.dailyTeam = new Chart(document.getElementById('dailyTeamChart').getContext('2d'), {
            ...
            options: {
                scales: {
                    ...
                    x: { ticks: { 
 color: '#ffffff' }, grid: { color: 'rgba(241, 241, 241, 0.2)' } }
                },
                plugins: { legend: { labels: { color: '#ffffff' } } }
            }
        });
        ...
        if (comparisonChartPeriod === 'daily') {
            ...
            const semesterEndMonth = month <= 6 ? '06' : '12';
    ...
    const updateAttendanceChart = () => {
        ...
        if (attendanceChartPeriod === 'daily') {
            if (evaluations[date]) {
                Object.entries(evaluations[date]).forEach(([id, ev]) => {
                    if (attendanceData[id] && ev['presenza-allenamento'] && parseInt(ev['presenza-allenamento'], 10) > 0) {
 ...
 dateObj = new Date(award.date);
    ...
    const updatePerformanceChart = () => {
        ...
        const chartData = {
            ...
            datasets: [{
                label: gpsFieldsForDisplay[selectedMetric] ||
 selectedMetric,
 ...
 metric, data: athleteValues, borderColor: 'rgba(217, 4, 41, 1)', backgroundColor: 'rgba(217, 4, 41, 0.2)', tension: 0.3, fill: true },
        ...
        Object.keys(radarMetrics).forEach(metric => {
            ...
            Object.values(dataToUse).forEach(playerData => {
                Object.values(playerData).forEach(sessions => {
                    sessions.forEach(session => {
                        const 
 value = parseFloat(session[metric] || 0);
 ...
 'N/A',
  ...
  pointHoverBackgroundColor: '#fff',
    ...
    elements.addAthleteBtn.addEventListener('click', () => {
        ...
        // ✅ Resetta anche i checkbox per Capitano e Vice Capitano
        document.getElementById('athlete-captain').checked = 
 false;
    ...
    elements.athleteGrid.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.edit-btn')) {
            ...
            // ✅ Aggiungi la gestione del Vice Capitano
            ...
            if (confirm(`Sei sicuro di voler eliminare ${athlete.name}? Tutti i suoi dati storici verranno rimossi.`)) {
                ...
                Object.keys(matchResults).forEach(matchId => {
                    ...
                    // ✅ MODIFICA: Filtra anche gli assist
    ...
    // ✅ MODIFICA: Aggiorna l'evento submit del form atleta per gestire `isViceCaptain`
    elements.athleteForm.addEventListener('submit', (e) => {
       ...
       role: document.getElementById('athlete-role').value.trim(),
            ...
            // ✅ Aggiungi la proprietà isViceCaptain
            ...
            avatar: avatarBase64 
 || (existingAthlete ? existingAthlete.avatar : '')
        };
        if (athleteId) {
            ...
            // ✅ Assicurati che `isViceCaptain` sia `false` per i nuovi atleti
            athleteData.isViceCaptain = athleteData.isViceCaptain ||
 false;
    ...
    elements.gpsForm.addEventListener('submit', (e) => {
        ...
        allGpsFields.forEach(field => {
            ...
            if(el) {
                ...
                if(value 
 !== '') gpsFormData[field] = el.type === 'number' ? parseFloat(value) : value;
            }
        });
        if (!sessionId) {
        ...
        const dist = parseFloat(gpsFormData.distanza_totale) ||
 0;
 ...
 * 60 + sec + cen / 100;
    ...
    elements.deleteDayDataBtn.addEventListener('click', () => {
        ...
        if (confirm(`Sei sicuro di voler eliminare TUTTI i dati (valutazioni, GPS e premi) del 
 giorno ${formattedDate}? L'azione è irreversibile.`)) {
            if (evaluations[date]) {
 ...
 evaluations[date][athleteId]) {
                ...
                saveData().then(() => {
                    evaluationModal.hide();
                    updateAllUI();
                    alert(`Dati di ${athlete.name} per il giorno 
 ${formattedDate} eliminati.`);
                });
            }
        }
    });
        ...
        if (e.target.id === 'gps-session-selector') {
            ...
            const populateGpsForm = (data) => {
             ...
             el.value = data[field];
                    ...
                    const totalSec = data.tempo_circuito_totale_s ||
 0;
                    form.querySelector('#gps-tempo_circuito_min').value = totalSec > 0 ? Math.floor(totalSec / 60) : '';
                    form.querySelector('#gps-tempo_circuito_sec').value = totalSec > 0 ?
                    ...
                    newDeleteBtn.addEventListener('click', () => {
                        if (confirm(`Sei sicuro di voler eliminare questa sessione?`)) {
                            if(gpsData[athleteId][session.date]){
                                gpsData[athleteId][session.date] = gpsData[athleteId][session.date].filter(s => String(s.id) 
 !== String(sessionId));
            ...
            const data = validSelections.map(selection => {
             ...
             Ora: gpsRecord.ora_registrazione || 'N/A',
                ...
                Object.entries(gpsFieldsForDisplay).forEach(([key, label]) => {
                    if (!['Data', 'Ora', 'Tipo'].includes(label)) 
 row[label] = gpsRecord[key] ?? 'N/A';
                });
                return row;
            });
            ...
            doc.autoTable({
                ...
                columnStyles: { 0: {cellWidth: 20}, 1: {cellWidth: 18}, 2: 
 {cellWidth: 15}, 3: {cellWidth: 20} }
            });
        ...
        if (dropZone) {
            ...
            if (athleteId) {
                if (dropZone.id === 'field-container' || dropZone.id === 'field-bench-area') {
                    const targetArray = dropZone.id === 'field-container' ?
 formationData.starters : formationData.bench;
        ...
        for (const key in chartInstances) {
            ...
            if (chart.options.scales) {
             ...
             Object.values(chart.options.scales).forEach(scale => {
                    ...
                    if 
 (scale.angleLines) scale.angleLines.color = 'rgba(241, 241, 241, 0.2)';
                });
            }
    ...
    const openMatchResultModal = (matchId = null) => {
        ...
        document.getElementById('assists-container').innerHTML = ''; // ✅ AGGIUNTA
 ...
 '';
            ...
            (match.assists || []).forEach(addAssistInput); // ✅ AGGIUNTA
    ...
    const addScorerInput = (scorer = {}) => {
        ...
        div.innerHTML = `<select class="form-select form-select-sm scorer-athlete" required><option value="">Seleziona atleta...</option>${athletes.map(a => `<option value="${a.id}" ${scorer.athleteId == a.id ? 'selected' : ''}>${a.name}</option>`).join('')}</select><input type="number" class="form-control form-control-sm scorer-minute" placeholder="Min" min="1" style="width: 80px;"
 value="${scorer.minute || ''}" required><button type="button" class="btn btn-sm btn-outline-danger remove-row-btn"><i class="bi bi-trash"></i></button>`;
        container.appendChild(div);
    };

    // ✅ NUOVA FUNZIONE: Aggiunge l'input per l'assist
    ...
    const addCardInput = (card = {}) => {
        ...
        const div = document.createElement('div');
        div.className = 'd-flex gap-2 align-items-center';
        div.innerHTML = `<select class="form-select form-select-sm card-athlete" required><option value="">Seleziona atleta...</option>${athletes.map(a => `<option value="${a.id}" ${card.athleteId == a.id ? 'selected' : ''}>${a.name}</option>`).join('')}</select><select class="form-select form-select-sm card-type" style="width: 120px;"
 required><option value="yellow" ${card.type === 'yellow' ? 'selected' : ''}>Giallo</option><option value="red" ${card.type === 'red' ?
 'selected' : ''}>Rosso</option></select><input type="number" class="form-control form-control-sm card-minute" placeholder="Min" min="1" style="width: 80px;"
 value="${card.minute || ''}" required><button type="button" class="btn btn-sm btn-outline-danger remove-row-btn"><i class="bi bi-trash"></i></button>`;
        container.appendChild(div);
    };
    ...
    document.getElementById('add-assist-btn').addEventListener('click', () => addAssistInput()); // ✅ AGGIUNTA
    ...
    elements.matchResultForm.addEventListener('submit', e => {
        ...
        const opponentScore = opponentScoreInput === '' 
 ? null : parseInt(opponentScoreInput);
        const matchData = {
            id: id,
 ...
 myTeamScore : opponentScore,
        ...
        // ✅ AGGIUNTA: Estrazione degli assist
