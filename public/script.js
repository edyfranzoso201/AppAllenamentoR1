...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ Aggiunta 'assenza-giustificata'
    const evaluationCategories = ['presenza-allenamento', 'serieta-allenamento', 'abbigliamento-allenamento', 'abbigliamento-partita', 'comunicazioni', 'doccia', 'assenza-giustificata'];
    ...
    const loadData = async () => {
        try {
            ...
            // ✅ Retrocompatibilità: Assicura che ogni valutazione abbia 'assenza-giustificata'
            Object.keys(evaluations).forEach(date => {
                Object.keys(evaluations[date]).forEach(athleteId => {
                    if (evaluations[date][athleteId]['assenza-giustificata'] === undefined) {
                        evaluations[date][athleteId]['assenza-giustificata'] = '0';
                    }
                });
            });
    ...
    // ✅ Escludi 'assenza-giustificata' dal punteggio
    const calculateAthleteScore = (evaluation) => !evaluation ? 0 : Object.keys(evaluation).filter(k => !['doccia', 'assenza-giustificata'].includes(k)).reduce((sum, key) => sum + parseInt(evaluation[key] || 0, 10), 0);
    ...
    // ✅ MODIFICA: Funzione aggiornata per conteggiare presenze e assenze giustificate
    const updateAttendanceChart = () => {
        const date = elements.evaluationDatePicker.value;
        if (!date) return;
        const attendanceData = {};
        athletes.forEach(a => attendanceData[String(a.id)] = { name: a.name, presence: 0, justifiedAbsence: 0 });
        if (attendanceChartPeriod === 'daily') {
            if (evaluations[date]) {
                Object.entries(evaluations[date]).forEach(([id, ev]) => {
                    if (attendanceData[id]) {
                        if (ev['presenza-allenamento'] && parseInt(ev['presenza-allenamento'], 10) > 0) {
                            attendanceData[id].presence = 1;
                        }
                        if (ev['assenza-giustificata'] && parseInt(ev['assenza-giustificata'], 10) > 0) {
                            attendanceData[id].justifiedAbsence = 1;
                        }
                    }
                });
            }
    ...
    elements.evaluationForm.addEventListener('submit', (e) => {
        ...
        // ✅ Salva tutte le categorie, inclusa 'assenza-giustificata'
