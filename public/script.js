...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ CORREZIONE: Modale matchResultModal con struttura corretta
                ...
                <div class="modal-body">
                    <form id="match-result-form">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-4">
                                <h5><i class="bi bi-person-check-fill"></i> Assist (GO Sport)</h5>
    ...
    // ✅ CORREZIONE: Inizializzazione corretta dei modali Bootstrap
    ...
    const elements = {
        ...
        topAssistersContainer: document.getElementById('top-assisters-container'),
    ...
    const saveData = async () => {
        ...
        try {
            ...
            // ✅ CORREZIONE: Salvataggio in localStorage come fallback
    ...
    const loadData = async () => {
        try {
            ...
            // ✅ CORREZIONE: Assicurarsi che tutti gli atleti abbiano la proprietà `isViceCaptain`
            athletes.forEach(athlete => {
                if (athlete.isViceCaptain === undefined) {
                    athlete.isViceCaptain = false;
                }
                // ✅ CORREZIONE: Assicurarsi che matchResults abbiano array assists
            ...
            // ✅ CORREZIONE: Caricamento da localStorage come fallback
            const localData = localStorage.getItem('coachDashboardData');
            if (localData) {
    ...
    // ✅ CORREZIONE: Funzione per creare elementi jersey
    ...
    const updateHomePage = () => {
        ...
        // ✅ CORREZIONE: Calcolo top performer corretto
        const now = new Date(); 
            ...
            if (dateStr.startsWith(currentMonthStr)) { 
                Object.entries(dailyEvals).forEach(([athleteId, evaluation]) => { 
                    if (monthlyScores[athleteId]) { 
                        monthlyScores[athleteId].score += calculateAthleteScore(evaluation); 
                    } 
                }); 
            } 
        }); 
    ...
    // ✅ CORREZIONE: Render athletes corretto
            ...
            card.innerHTML = `
                <div class="card athlete-card">
                    <div class="card-body athlete-card-clickable" data-athlete-id="${athlete.id}">
                        ...
                        <div>
                            <h5 class="card-title">${athlete.name} ${athlete.isCaptain ? '<i class="bi bi-star-fill is-captain"></i>' : ''} ${vcIcon}</h5>
    ...
    // ✅ CORREZIONE: Render calendar corretto
    ...
    // ✅ CORREZIONE: Render match results corretto
    const renderMatchResults = () => {
        ...
        sortedMatches.forEach(match => {
            ...
            const cardContent = `
                <div class="card h-100 match-result-item ${cardClass}" data-match-id="${match.id}" style="cursor: pointer;">
                    <div class="card-body p-2">
                        ...
                        ${myTeamAssists ? `<div class="assists-list"><i class="bi bi-person-check-fill"></i> Assist: <strong style="color: var(--primary-blue);">${myTeamAssists}</strong></div>` : ''}
                    </div>
                </div>`;
            colDiv.innerHTML = cardContent;
            elements.matchResultsContainer.appendChild(colDiv);
            
            // ✅ CORREZIONE: Aggiunta corretta event listener per aprire modale partita
            ...
            // ✅ CORREZIONE: Aggiunta event listener per pulsante modifica
            colDiv.querySelector('.edit-match-btn').addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                openMatchResultModal(match.id);
            });
    ...
    const renderCardsSummary = () => {
        ...
        const allCards = Object.values(matchResults).flatMap((match) => 
            (match.cards || []).map((card, cardIndex) => ({ ...card, date: match.date, uniqueId: `${match.id}-${cardIndex}` }))
    ...
    // ✅ CORREZIONE: Render top assisters
    const renderTopAssisters = () => {
        ...
        if (sortedAssisters.length === 0) {
            elements.topAssistersContainer.innerHTML = '<p class="text-muted">Nessun assist registrato.</p>';
            return;
        }
        
        let ol = '<ol class="list-group list-group-numbered">';
        sortedAssisters.forEach(assister => {
            ol += `<li class="list-group-item d-flex justify-content-between align-items-center" style="background: transparent; border-color: var(--border-color);">${assister.name}<span class="badge bg-primary rounded-pill">${assister.assists}</span></li>`;
        });
        ol += '</ol>';
        elements.topAssistersContainer.innerHTML = ol;
    };

    // ✅ CORREZIONE: Update match analysis chart
    const updateMatchAnalysisChart = () => {
        const opponentFilter = elements.matchOpponentFilter.value;
        const periodBtn = elements.matchPeriodToggle.querySelector('.btn.active');
        const period = periodBtn ? periodBtn.dataset.period : 'monthly';
        ...
        if (opponentFilter !== 'all') {
            ...
            filteredMatches = filteredMatches.filter(m => m.opponent