...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (resto del codice identico al tuo file funzionante, con le seguenti modifiche cruciali)
    ...
    // ✅ Calcola punteggio escludendo "Assenza Giustificata" (valore 4)
    const calculateAthleteScore = (evaluation) => {
        ...
        for (const [key, value] of Object.entries(evaluation)) {
            ...
            if (key === 'presenza-allenamento' && numValue === 4) {
                continue; // Escludi assenza giustificata
            }
            total += numValue || 0;
        }
        return total;
    ...
    // ✅ Aggiorna il conteggio "Atleti in Rosa" escludendo gli ospiti
    ...
    // ✅ Aggiorna il grafico "Report Presenze" con torrino verde per le assenze giustificate
    const updateAttendanceChart = () => {
        const date = elements.evaluationDatePicker.value;
        if (!date) return;

        // ✅ Filtra solo gli atleti NON ospiti
        const officialAthletes = athletes.filter(a => !a.isGuest);
            ...
            if (evaluations[date]) {
                Object.entries(evaluations[date]).forEach(([id, ev]) => {
                    if (attendanceData[id]) {
                        const value = parseInt(ev['presenza-allenamento'], 10);
                        if (value > 0 && value < 4) attendanceData[id].presence = 1;
                        if (value === 4) attendanceData[id].justifiedAbsence = 1;
                    }
                });
            }
        } else {
            // ... (logica analoga per weekly, monthly, etc. — conteggio incrementale)
            let dateRange = [];
            ...
            dateRange.forEach(d => {
                if (evaluations[d]) {
                    Object.entries(evaluations[d]).forEach(([id, ev]) => {
                        if (attendanceData[id]) {
                            const value = parseInt(ev['presenza-allenamento'], 10);
                            if (value > 0 && value < 4) attendanceData[id].presence++;
                            if (value === 4) attendanceData[id].justifiedAbsence++;
                        }
                    });
                }
            });
            ...
            data: {
                ...
                datasets: [
                    ...
                    },
                    {
                        ...
                        backgroundColor: 'rgba(0, 100, 0, 0.8)' // Verde
                    }
                ]
            },
            options: {
    ...
    // ✅ Assicurati che tutti gli atleti abbiano `isGuest = false` se non definito
    const loadData = async () => {
        try {
            ...
            // ✅ Retrocompatibilità per `isGuest` e `isViceCaptain`
            athletes.forEach(athlete => {
                if (athlete.isViceCaptain === undefined) athlete.isViceCaptain = false;
                if (athlete.isGuest === undefined) athlete.isGuest = false;
            });

            // ✅ Retrocompatibilità per "Assenza Giustificata": assicura che il campo esista
            Object.keys(evaluations).forEach(date => {
                Object.keys(evaluations[date]).forEach(athleteId => {
                    if (evaluations[date][athleteId]['presenza-allenamento'] === undefined) {
                        evaluations[date][athleteId]['presenza-allenamento'] = '0';
                    }
                });
            });
    ...
    // ... (resto del codice identico: saveData, renderAthletes con `.guest-athlete`, ecc.)
});