...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculateAthleteScore = (evaluation) => {
        ...
        return Object.keys(evaluation)
            ...
            .reduce((sum, key) => {
                ...
                if (val === 'J') return sum; // ✅ Assenza giustificata non conta
    ...
    // ✅ Filtra gli atleti con almeno una presenza o assenza giustificata
    const athletesWithData = officialAthletes.filter(a => presenceCounts[a.id] > 0 || justifiedCounts[a.id] > 0);

    // ✅ Prepara i dati per il grafico
    const labels = athletesWithData.map(a => a.name);
    const presenze = athletesWithData.map(a => presenceCounts[a.id] || 0);
    const assenzeGiust = athletesWithData.map(a => justifiedCounts[a.id] || 0);
    ...
    chartInstances.attendance = new Chart(document.getElementById('attendanceChart').getContext('2d'), {
        ...
        options: {
            scales: {
                y: {
                    beginAtZero: true,
                    ticks: { color: '#ffffff', stepSize: 1 }, // ✅ stepSize: 1 per leggibilità
