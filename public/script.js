...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ AGGIUNTO: campo "Atleta Proveniente da Altra Annata" nel modale atleta
    modalsContainer.innerHTML = `
        ...
        <!-- ✅ TUTTI GLI ALTRI MODALI (gpsModal, sessionModal, matchResultModal, passwordModal) -->
        <!-- Copiati identici dal file originale, non modificati -->
        <!-- ... (mantieni esattamente come nel tuo script.js originale) ... -->
    ...
    // ✅ Inizializza tutti i modali (come nel tuo codice originale)
    ...
    // ✅ Listener per mostrare/nascondere campo annata
    ...
    // ✅ TUTTO IL RESTO DEL TUO CODICE ORIGINALE, INALTERATO
    // ... (mantieni esattamente come nel tuo script.js originale) ...
    // Le uniche modifiche sono riportate qui sotto
    ...
    // ✅ Funzioni di autenticazione (inalterate)
    ...
    // ✅ ACCESS_PASSWORD e auth (inalterate)
    ...
    // ✅ loadData (inalterata, tranne aggiunta isGuest se manca)
    ...
    // ✅ saveData, migrateGpsData (inalterate)
    ...
    const renderAthletes = () => {
        ...
        athletes.forEach(athlete => {
            ...
            if (athlete.isGuest) {
                ...
                guestBadge = `<span class="badge guest-badge ms-1">ospite ${seasonLabel}</span>`;
            }
    ...
    // ✅ updateHomePage: conteggio separato guest
    const updateHomePage = () => {
        const totalAthletes = athletes.length;
        const guestAthletes = athletes.filter(a => a.isGuest).length;
        const coreAthletes = totalAthletes - guestAthletes;
        elements.homeTotalAthletes.innerHTML = `
            ${totalAthletes} <small class="text-muted">(di cui ${guestAthletes} ospiti)</small>
        `;
    ...
    // ✅ athleteForm submit: salva isGuest e guestFromSeason
    ...
    // ✅ Modifica atleta: popola i campi guest
    elements.athleteGrid.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.edit-btn')) {
            ...
            document.getElementById('athlete-guest-season').value = athlete.guestFromSeason || '';
            document.getElementById('guest-season-row').style.display = (athlete.isGuest) ? 'block' : 'none';
        ...
        // ... resto del gestore click inalterato ...
    ...
    // ✅ TUTTE LE ALTRE FUNZIONI (renderFormation, renderMatchResults, ecc.) SONO INALTERATE
    // Perché gli atleti guest sono trattati come normali atleti

    // --- FUNZIONI DI UTILITÀ (inalterate) ---
    ...
    // ✅ Tutte le altre funzioni (renderFormation, renderCalendar, ecc.) rimangono esattamente come nel tuo codice originale
