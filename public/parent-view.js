(function () {
  ...
  const createEventCard = (eventDate, eventDateObj, eventData, response, isLocked) => {
    ...
    const dateStr = eventDateObj.toLocaleDateString('it-IT', { day: '2-digit', month: 'long', year: 'numeric' });
    ...
    return `
      <div class="card mb-3 ${isLocked ? 'opacity-75' : ''}">
        <div class="card-header bg-${typeColor} text-white">
          <h5 class="mb-0"><i class="bi bi-${eventData.type === 'Partita' ? 'trophy' : 'dribbble'}"></i> ${eventData.type}</h5>
        </div>
        <div class="card-body">
  ...
  const handleResponse = async (e) => {
    ...
    try {
      ...
      alert.innerHTML = '<i class="bi bi-check-circle-fill"></i> Risposta salvata con successo! <button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
      ...
      e.currentTarget.innerHTML = responseType === 'Si' ? '<i class="bi bi-check-circle-fill"></i> SI, sarò presente' : '<i class="bi bi-x-circle-fill"></i> NO, sarò assente';
    }
  };
