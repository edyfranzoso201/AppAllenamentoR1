(function () {
  ...
  window.renderCalendarTable = function () {
    ...
    const eventDates = Object.keys(events).filter(date => new Date(date) >= firstDayOfMonth).sort();
    ...
    thead.innerHTML = '<th style="width: 40px;"></th><th style="min-width: 150px;">Atleta</th><th style="width: 100px;">Link</th>' + eventDates.map(date => {
      const d = new Date(date);
      const dayName = d.toLocaleDateString('it-IT', { weekday: 'short' });
      const dayNum = d.getDate();
      const month = d.toLocaleDateString('it-IT', { month: 'short' });
      return '<th class="text-center" style="min-width: 80px; writing-mode: vertical-rl; transform: rotate(180deg);"><div class="py-2">' + dayName + ' ' + dayNum + month + '</div></th>';
    }).join('');
    calendarElements.eventDetailsRow.innerHTML = '<th colspan="3" class="text-center bg-secondary">Dettagli</th>' + eventDates.map(date => {
      const event = events[date];
      return '<th class="text-center bg-secondary event-header-cell" data-date="' + date + '" style="cursor: pointer;" title="Clicca per modificare"><div class="small">' + event.type + '</div><div class="small">' + (event.time || '-') + '</div><div class="small text-muted">' + (event.lockDaysBefore || 0) + 'gg</div></th>';
    }).join('');
    ...
    calendarElements.calendarTbody.innerHTML = nonGuestAthletes.map((athlete, index) => {
      ...
      return '<tr><td class="text-center">' + (index + 1) + '</td><td><strong>' + athlete.name + '</strong></td><td class="text-center"><button class="btn btn-sm btn-outline-primary copy-link-btn" data-link="' + athleteLink + '" title="Copia link"><i class="bi bi-link-45deg"></i></button></td>' + eventDates.map(date => {
  ...
  const deleteOldMonths = async () => {
    const today = new Date();
    const firstDayOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
    const eventsToDelete = Object.keys(window.calendarEvents || {}).filter(date => new Date(date) < firstDayOfMonth);
    const responsesToDelete = Object.keys(window.calendarResponses || {}).filter(date => new Date(date) < firstDayOfMonth);
    if (eventsToDelete.length === 0 && responsesToDelete.length === 0) {
      alert('Non ci sono mesi passati da eliminare.');
      return;
    }
    if (!confirm('Eliminare ' + eventsToDelete.length + ' eventi e ' + responsesToDelete.length + ' date con risposte dai mesi precedenti?')) return;
  ...
  const viewAllResponses = () => {
    ...
    Object.keys(events).sort().forEach(eventDate => {
      ...
      const dateStr = eventDateObj.toLocaleDateString('it-IT', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' });
      ...
      html += '<div class="card mb-3"><div class="card-header bg-primary text-white"><strong>' + event.type + '</strong> - ' + dateStr + (event.time ? ' <span class="ms-2">' + event.time + '</span>' : '') + '</div><div class="card-body"><div class="row mb-3"><div class="col-md-4"><strong>Risposte</strong> ' + totalResponses + '/' + totalAthletes + '</div><div class="col-md-4 text-success"><strong>Presenti</strong> ' + siCount + '</div><div class="col-md-4 text-danger"><strong>Assenti</strong> ' + noCount + '</div></div>' + (Object.keys(eventResponses).length > 0 ? '<div class="table-responsive"><table class="table table-sm"><thead><tr><th>Atleta</th><th>Risposta</th><th>Motivazione</th><th>Data/Ora</th></tr></thead><tbody>' + Object.entries(eventResponses).map(entry => {
        const athleteId = entry[0];
        const resp = entry[1];
        const athlete = athletes.find(a => String(a.id) === String(athleteId));
        const timestamp = new Date(resp.timestamp);
        return '<tr><td>' + (athlete?.name || 'Sconosciuto') + '</td><td>' + (resp.presenza === 'Si' ? 'SI' : 'NO') + '</td><td>' + resp.motivazione + '</td><td class="text-muted small">' + timestamp.toLocaleDateString('it-IT') + ' ' + timestamp.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' }) + '</td></tr>';
      }).join('') + '</tbody></table></div>' : '<p class="text-muted">Nessuna risposta ancora</p>') + '</div></div>';
    });
  ...
  const showAthleteLinks = () => {
    ...
    const html = nonGuestAthletes.map(athlete => {
      ...
      return '<div class="card mb-2"><div class="card-body py-2"><div class="row align-items-center"><div class="col-md-4"><strong>' + athlete.name + '</strong></div><div class="col-md-6"><input type="text" class="form-control form-control-sm" value="' + link + '" readonly></div><div class="col-md-2"><button class="btn btn-sm btn-primary w-100 copy-link-btn" data-link="' + link + '"><i class="bi bi-clipboard"></i> Copia</button></div></div></div></div>';
    }).join('');
