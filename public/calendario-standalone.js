...
window._renderParentDocsSection = function(athleteId, annataId, allDocs) {
  ...
  _DOC_TYPES.forEach(function(dt) {
    ...
    if (doc && doc.url) {
      ...
      var html_nota = doc.nota ? '<div style="color:#94a3b8;font-size:0.73rem;margin-top:2px;">📝 ' + doc.nota + '</div>' : '';
...
const _DOC_TYPES = [
  ...
  { key: 'liberatoria',  label: '🎥 Liberatoria Foto e Video' },
  { key: 'generico1',    label: '📎 Doc. Generico 1' },
  { key: 'generico2',    label: '📎 Doc. Generico 2' },
  { key: 'generico3',    label: '📎 Doc. Generico 3' },
  { key: 'generico4',    label: '📎 Doc. Generico 4' },
  { key: 'generico5',    label: '📎 Doc. Generico 5' },
  { key: 'generico6',    label: '📎 Doc. Generico 6' }
];
...
window._showDocUploadPanel = function(athleteId, annataId, existingDocs) {
  ...
  _DOC_TYPES.forEach(function(dt) {
    ...
    var nota = (existing[dt.key] && existing[dt.key].nota) ? existing[dt.key].nota : '';
    ...
    html += '<div style="margin-bottom:14px;background:#0d1b2a;border:1px solid #2d4a8a;border-radius:8px;padding:10px;">'
      ...
      + 'style="width:100%;padding:9px 12px;background:#060f1e;border:1px solid #3b5a9d;border-radius:6px;color:#e2e8f0;font-size:0.82rem;box-sizing:border-box;margin-bottom:6px;">'
      + '<input type="text" id="doc-nota-' + dt.key + '" value="' + (nota || '') + '" '
      + 'placeholder="Nota breve (opzionale, es. scadenza, tipo documento...)" maxlength="100" '
      + 'style="width:100%;padding:7px 12px;background:#060f1e;border:1px solid #2d4a8a;border-radius:6px;color:#94a3b8;font-size:0.78rem;box-sizing:border-box;">'
      + '</div>';
  });
...
window._saveAthleteDoc = async function(athleteId, annataId) {
  ...
  _DOC_TYPES.forEach(function(dt) {
    ...
    var notaInput = document.getElementById('doc-nota-' + dt.key);
    ...
    if (url) {
      docs[dt.key] = { url: url, nota: nota, uploadedAt: new Date().toISOString(), uploadedBy: 'genitore' };
      hasAny = true;
    }
  });
