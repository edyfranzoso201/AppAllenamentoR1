...
async function handleLogin(req, res) {
  try {
    ...
    // Registra accesso su Redis usando get/set
    try {
      const logEntry = {
      ...
      const updated = [logEntry, ...arr].slice(0, 500);
      await kv.set('access_log', updated);
      console.log(`✅ Log scritto: ${user.username}, tot=${updated.length}`);
    } catch(logErr) {
      console.error('❌ Access log ERRORE:', logErr.message, logErr.stack);
    }
...
async function handleAccessLog(req, res) {
  try {
    ...
    console.error('Access log read error:', error);
    return res.status(500).json({ message: 'Errore lettura log', logs: [] });
  }
}
...
async function handleData(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const posts       = await kv.get(`data:${annataId}:posts`)       || [];
      const postImages  = await kv.get(`data:${annataId}:postImages`)  || {};
    ...
    try {
      ...
      if (allData.posts !== undefined)       await kv.set(`data:${annataId}:posts`,      allData.posts);
      if (allData.postImages !== undefined)  await kv.set(`data:${annataId}:postImages`, allData.postImages);
      if (allData.globalPosts !== undefined) await kv.set(`global:posts`, allData.globalPosts);
...
async function handleGare(req, res) {
  // societyId da query (GET) o body (POST)
  const societyId = (req.query && req.query.societyId)
  ...
  const keyGare    = `society:${societyId}:gare`;
  const keySquadre = `society:${societyId}:squadre`;

  if (req.method === 'GET') {
    const gare    = (await kv.get(keyGare))    || [];
    const squadre = (await kv.get(keySquadre)) || [];
    console.log(`✅ GET /api/gare - societyId=${societyId} gare=${gare.length} squadre=${squadre.length}`);
    return res.status(200).json({ gare, squadre });
  }

  if (req.method === 'POST') {
    if (req.body.gare    !== undefined) await kv.set(keyGare,    req.body.gare);
    if (req.body.squadre !== undefined) await kv.set(keySquadre, req.body.squadre);
    console.log(`✅ POST /api/gare - societyId=${societyId}`);
    return res.status(200).json({ success: true });
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ============================================
    if (path === '/api/gare') {
      return await handleGare(req, res);
    }
    // Route non trovata
